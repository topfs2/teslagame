...
namespace Tesla.GFX
{
	...
	public class SDLWindow
	{
		...
		private bool quitFlag, Fullscreen;
		private int width, height, bpp, flags;
		int lastTickCount;
		private string Name;
		
		private Point2f rot;
		private Point2f lastMousePos, currMousePos;
		...
		{
			currMousePos = new Point2f(0.0f, 0.0f);
			lastMousePos = new Point2f(0.0f, 0.0f);
			this.width = width;
			this.height = height;
			this.bpp = bpp;
			Name = name;
			...
			try
			{
				...
				Sdl.SDL_WarpMouse((short)(this.width / 2), (short)(this.height / 2));
			}
			catch
			{
        ...
        {			
			...
			{
				Console.Out.WriteLine("[X, Y] [" + evt.motion.x + "," + evt.motion.y + "] [relX, relY] [" + evt.motion.xrel + "," + evt.motion.yrel + "] ");
				//lastMousePos = currMousePos;
				currMousePos = new Point2f(evt.motion.x, evt.motion.y);
				Point2f d = currMousePos.subtract(new Point2f((short)(this.width / 2), (short)(this.height / 2)));
				float sensativity = 30.0f;
				camera.rotateX(-d.y * frameTime * sensativity);
				camera.rotateY(d.x * frameTime * sensativity);
				Sdl.SDL_WarpMouse((short)(this.width / 2), (short)(this.height / 2));
        ...
        private void InitGL()
        {
			camera = new Camera(new Point3f(0.0f, 0.0f, 0.0f));
            try
            {
